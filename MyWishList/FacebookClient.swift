...
class FacebookClient : HTTPClient {
    ...
    func getMe(completionHandler: (result: User?, error: NSError?) -> Void) {
        ...
        request.startWithCompletionHandler {
            ...
            completionHandler(result: User(fromJson: result), error: nil)
        }
    }
    
    func getFriends(completionHandler: (result: [User], error: NSError?) -> Void) {
        let params = ["fields": "id,name,picture", "limit": "\(MAX_PAGE_SIZE)"]
        let request = FBSDKGraphRequest(graphPath: "/me/friends", parameters: params)
        request.startWithCompletionHandler {
            connection, result, error in
            guard let result = result as? [String : AnyObject], data = result["data"] as? [[String: AnyObject]] where data.count > 0 else {
                completionHandler(result: [], error: error)
                return
            }
            completionHandler(result: self.mapFriends(fromJsonArray: data), error: nil)
    ...
    private func getAdditonalFriends(url: String, completionHandler: (result: [User], error: NSError?) -> Void) {
        let httpHeaders: [String : String] = [:]
        get(url, httpHeaders: httpHeaders) {
            data, response, error in
            let httpResponse = self.buildJsonResponse(data, response: response, error: error)
            guard let result = httpResponse.parsedResult as? [String : AnyObject], data = result["data"] as? [[String: AnyObject]] where data.count > 0 else {
                completionHandler(result: [], error: error)
                return
            }
